...
export class FieldCriteria<Entity extends ObjectLiteral> extends CriteriaBase<Entity> {
    constructor(
    ...
    apply(queryBuilder: SelectQueryBuilder<Entity>): void {
        ...
        switch (this.operator) {
            ...
            case ComparisonOperator.LESS_THAN_OR_EQUAL:
                queryBuilder.andWhere(`${this.field} ${this.operator} :${paramName}`, { [paramName]: this.value });
                break;
            ...
            case ComparisonOperator.BETWEEN:
                if (this.value !== undefined && this.secondValue !== undefined) {
                    ...
                    queryBuilder.andWhere(`${this.field} BETWEEN :${paramName} AND :${paramName2}`, {
    ...
    private generateParamName(): string {
        return this.field.replace(/\./g, '_') + '_' + Math.random().toString(36).substr(2, 9);
    }
}

// Raw SQL criteria for complex conditions
export class RawCriteria<Entity extends ObjectLiteral> extends CriteriaBase<Entity> {
    constructor(
...
export class PaginationCriteria<Entity extends ObjectLiteral> extends CriteriaBase<Entity> {
...
export class CriteriaCombiner<Entity extends ObjectLiteral> extends CriteriaBase<Entity> {
    private criteriaList: Array<{ criteria: CriteriaBase<Entity>; operator: LogicalOperator }> = [];

    constructor(initialCriteria?: CriteriaBase<Entity>) {
        super();
        if (initialCriteria) {
            this.criteriaList.push({ criteria: initialCriteria, operator: LogicalOperator.AND });
        }
    }
    ...
    apply(queryBuilder: SelectQueryBuilder<Entity>): void {
        ...
        for (let i = 1; i < this.criteriaList.length; i++) {
            ...
            if (operator === LogicalOperator.OR) {
                ...
                queryBuilder.orWhere(`(${subQuery.getQuery()})`, subQuery.getParameters());
            } else {
    ...
    static create<T extends ObjectLiteral>(criteria?: CriteriaBase<T>): CriteriaCombiner<T> {
...
export class CriteriaBuilder {
    static equals<T extends ObjectLiteral>(field: string, value: any): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.EQUALS, value);
    }

    static notEquals<T extends ObjectLiteral>(field: string, value: any): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.NOT_EQUALS, value);
    }

    static like<T extends ObjectLiteral>(field: string, value: string): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.LIKE, value);
    }

    static ilike<T extends ObjectLiteral>(field: string, value: string): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.ILIKE, value);
    }

    static greaterThan<T extends ObjectLiteral>(field: string, value: number | Date): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.GREATER_THAN, value);
    }

    static greaterThanOrEqual<T extends ObjectLiteral>(field: string, value: number | Date): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.GREATER_THAN_OR_EQUAL, value);
    }

    static lessThan<T extends ObjectLiteral>(field: string, value: number | Date): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.LESS_THAN, value);
    }

    static lessThanOrEqual<T extends ObjectLiteral>(field: string, value: number | Date): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.LESS_THAN_OR_EQUAL, value);
    }

    static in<T extends ObjectLiteral>(field: string, values: any[]): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.IN, values);
    }

    static notIn<T extends ObjectLiteral>(field: string, values: any[]): FieldCriteria<T> {
    ...
    static between<T extends ObjectLiteral>(field: string, startValue: any, endValue: any): FieldCriteria<T> {
        return new FieldCriteria<T>(field, ComparisonOperator.BETWEEN, startValue, endValue);
    }

    static raw<T extends ObjectLiteral>(condition: string, parameters: Record<string, any> = {}): RawCriteria<T> {
        return new RawCriteria<T>(condition, parameters);
    }

    static pagination<T extends ObjectLiteral>(pagination: PaginationDto): CriteriaBase<T> {
